...
class Bot {
  ...
  cmd(payload) { this.ws.send(JSON.stringify({ t: 'cmd', ...payload })); }
...
async function main() {
  ...
  check('five bots joined', bots.every((b) => b.pid > 0));
  ...
  await sleep(600);
  check('party of five formed', tank.self?.party?.members?.length === 5,
  ...
  await sleep(600);
  check('all at level 10', bots.every((b) => b.self?.lv === 10), JSON.stringify(bots.map((b) => b.self?.lv)));
  ...
  await sleep(800);
  check('all five in the dungeon', bots.every((b) => (b.self?.x ?? 0) > 600),
    JSON.stringify(bots.map((b) => Math.round(b.self?.x ?? 0))));
  const zs = bots.map((b) => b.self?.z ?? 0);
  check('all in the SAME instance', Math.max(...zs) - Math.min(...zs) < 120, JSON.stringify(zs.map(Math.round)));
  ...
  for (const b of [pala, priest, mage, hunter]) b.cmd({ cmd: 'dev_give', item: 'spring_water', count: 20 });
  await sleep(400);
  ...
  while (Date.now() - start < 480_000 && !bossDead) {
    ...
    for (const b of bots) {
      ...
      if (healers.has(b.pid)) {
        ...
        if (b.cls === 'priest' && (b.self.gcd ?? 0) <= 0 && !b.self.cast && tank.self && !tank.self.dead
      ...
      if (target) {
        ...
        if (d > meleeRange) {
          ...
          if ((b.self.gcd ?? 0) <= 0 && !b.self.cast) {
            if (b.cls === 'warrior' && (b.self.res ?? 0) >= 15) b.cmd({ cmd: 'cast', ability: 'heroic_strike' });
            if (b.cls === 'paladin' && (b.self.res ?? 0) >= 30) b.cmd({ cmd: 'cast', ability: 'judgement' });
            if (b.cls === 'mage' && (b.self.res ?? 0) >= 30) b.cmd({ cmd: 'cast', ability: 'fireball' });
            if (b.cls === 'hunter' && (b.self.res ?? 0) >= 25) b.cmd({ cmd: 'cast', ability: 'arcane_shot' });
          }
        }
      } else {
  ...
  console.log('final party state:', JSON.stringify(bots.map((b) => ({ n: b.name, hp: b.self?.hp, dead: b.self?.dead }))));
