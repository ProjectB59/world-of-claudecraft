...
//
// `IWorld` (src/world_api.ts, 169 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (169 / 42 / 127)
...
// The 169 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 42 `data` + 127 `method` (read-returning + command-void + async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    expect(METHOD_MEMBERS.length).toBe(127);
  });
...
//   (4) their union, sorted, equals the pinned 169-name IWORLD_MEMBERS set (a member
